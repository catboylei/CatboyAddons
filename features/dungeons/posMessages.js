...
register("step", () => {
    ...
    if (!Settings()[pos.replaceAll(' ', '')]) return // this is pretty scuffed but its a lot better than having one if statement per hard coded value...
...
register('renderWorld', () => {
    ...
    for (let key in data.posMessages) {
        ...
        if (!sent || !Settings()[key.replaceAll(' ', '')]) continue
