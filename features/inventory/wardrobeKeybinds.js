...
limeDye.itemStack.func_77964_b(10) // yes i have to do this to make the dye colored idfk
...
register("guiKey", (key, _, gui, event) => {
    if (!Settings().wardrobeHotkeys || !Settings().toggle) return
    if (!(gui instanceof net.minecraft.client.gui.inventory.GuiChest)) return; // return if not in a chest gui
    
    key = parseInt(key)
    if (!isWardrobeOpen || !(key > 0 && key < 10 )) return // return if not in wardrobe gui
    ...
    sendWindowClick(gui.field_147002_h.field_75152_c, slotIndex, 0, 0) // i love stealing utils from cyan
